...
- [ ] **HIGH / Large** — Implement the document-segmentation module designed in `docs/pipeline/09_chunking.md`. Add a `DocumentSegmenter` ABC with `NewsSegmenter` / `ResearchSegmenter` / `TranscriptSegmenter` / `FilingSegmenter` implementations splitting on natural boundaries (sections, speaker turns, Item numbers). Unlocks earnings transcripts, research reports, and 10-K filings — i.e. the *non-news* long-form content where financial entity density is highest.
