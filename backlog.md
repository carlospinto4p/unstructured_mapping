...
- [x] **MEDIUM / Medium** — Cold-start benchmarking CLI. Done in v0.44.0: `cli/benchmark_cold_start.py` loads a labelled JSON file, runs the articles through cold-start and/or KG-driven pipelines (each against a throwaway SQLite DB), and reports per-mode precision / recall / F1 against the ground-truth `(canonical_name, entity_type)` labels. Also reports token spend via the v0.43.0 scorecard.
