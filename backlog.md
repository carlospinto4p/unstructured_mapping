...
- [ ] **HIGH / Small** — N+1 entity lookups in `pipeline/resolution.py:345-365` (`_collect_candidates`). Each unique mention candidate ID triggers a separate `store.get_entity(eid)` round-trip. At ~30–50 candidates per chunk × many chunks per article × 5000 articles, this dominates resolution latency. Fix: add `KnowledgeStore.get_entities(ids: Iterable[str]) -> dict[str, Entity]` using `WHERE entity_id IN (...)` and have `_collect_candidates` fetch in one call.
- [ ] **HIGH / Medium** — Per-row `commit()` in `knowledge_graph/_entity_mixin.py:134` (`save_entity`) forces one ACID round-trip per write. `import_with_dedup` in `cli/_seed_helpers.py` writes ~2000 entities per populate run — that's 2000 commits where 1 would do. Fix: add a `KnowledgeStore.transaction()` context manager and wrap the loops in `import_with_dedup`, `save_relationships`, and any other batch writer. Expected populate-run speedup 10-50×.
- [ ] **MEDIUM / Small** — Missing composite index `(canonical_name COLLATE NOCASE, entity_type)` in `knowledge_graph/storage.py:166`. `exists_by_name_and_type` uses `find_by_name` (hits `idx_entity_name` on name only) then filters by type in Python. Add `CREATE INDEX IF NOT EXISTS idx_entity_name_type ON entities (canonical_name COLLATE NOCASE, entity_type)` so SQLite filters both columns at once.
- [ ] **MEDIUM / Small** — `orchestrator._process_article` calls `store.has_document_provenance(doc_id)` per article (orchestrator.py:404). For a 1000-article run this is 1000 queries. Fix: add `KnowledgeStore.get_documents_with_provenance(ids: Iterable[str]) -> set[str]`, call it once before the loop, then check set membership in memory.
- [ ] **MEDIUM / Small** — `wikidata_seed._already_imported` uses `store.find_by_alias(qid)` which fetches full `Entity` objects just to check existence (`cli/wikidata_seed.py:152-159`). Add `KnowledgeStore.alias_exists(alias: str) -> bool` doing `SELECT 1 ... LIMIT 1` and use it from `_already_imported`. Smaller gain than #1/#2 but free along the same code path.
- [ ] **MEDIUM / Small** — Detector initialisation scans the whole KG via `store.find_entities_by_status(ACTIVE)` with no limit (orchestrator.py:227). Fine today (~2k entities) but unbounded — a 10k+ KG will slow every pipeline instantiation. Either cap the query (`limit=5000` with a documented ceiling) or move to lazy/streaming trie construction.
- [ ] **LOW / Small** — Add `UNIQUE` index on `articles.document_id` in `web_scraping/storage.py:23`. Currently `INSERT OR IGNORE` relies on the `url` primary key; syndicated content with different URLs but the same canonical `document_id` can insert twice. Less a perf fix than a correctness-by-construction guard, but sidesteps any Python-side dedup loop that would otherwise grow.
