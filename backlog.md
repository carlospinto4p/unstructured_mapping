...
- [ ] **MEDIUM / Small** — Consolidate `get_*` vs `find_*` lookup naming. Today `get_entity` / `get_entities` return single-or-batched-by-id, but `find_by_name` / `find_by_alias` / `find_recent_mentions` / `get_relationships` / `find_relationships_by_type` mix the two prefixes for similar shapes. Pick a rule (`get_*` = keyed exact lookup, `find_*` = filtered search) and rename the outliers. Mostly a rename pass with deprecation aliases for one or two release cycles.
