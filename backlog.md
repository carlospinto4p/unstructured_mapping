...
- [ ] **HIGH** — LLM pass 1 (2a): prompt builder — `pipeline/prompts.py` with system prompt for pass 1, `build_kg_context_block(candidates)` producing the numbered text format from `llm_interface.md` § "KG context block format", and `build_pass1_user_prompt(kg_block, chunk_text, prev_entities)`. Pure string construction, no LLM call.
