...
- [ ] **MEDIUM / Small** — Expose LLM token usage on `LLMProvider`. Requires interface addition + per-backend implementation (Ollama, Claude), then plumbing through `resolver.resolve` / `extractor.extract` into the run metrics accumulator. Enables cost tracking in the scorecard.
