...
- [x] **MEDIUM / Medium** — Define a `KnowledgeStoreProtocol` (or internal Protocol) in `knowledge_graph/_helpers.py` so mixins can type their host-class dependencies. Eliminates ~8 `# type: ignore[attr-defined]` in `_entity_mixin.py:371,372,394,402,518` and `_provenance_mixin.py:212`.
