...
- [x] **MEDIUM / Medium** — Entity-centric subgraph extraction. Shipped as `cli/subgraph.py`: accepts `--entity-id` or `--name`, walks the k-hop frontier in both directions via `find_relationships`, and emits a deterministic JSON payload with the root entity, neighbours, edges, and the documents that justify each edge. Supports `--min-confidence` to drop weak edges before the BFS expands through them.
